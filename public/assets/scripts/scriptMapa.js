const map = L.map('map').setView([-12.1102137, -76.982390], 15)
...
function crearZonasPeligro() {
    leerJson().then(value => {
        for (let jsonObj of value) {
            ...
            let texto = "Zona " + String.fromCharCode(value.indexOf(jsonObj) + 65);

            texto += "\nPeligrosidad: " + jsonObj.peligrosidad;

            L.marker([jsonObj.lng, jsonObj.lat]).addTo(map).bindPopup(texto).openPopup();
        }
    });
...
function leerJson() {
    return fetch("assets/scripts/zonas.json")
